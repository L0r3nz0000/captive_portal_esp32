...
class MyApp:
    ...
    async def handle_http_connection(self, reader, writer):
        ...
        # Handle the request
        if "GET /generate_204" in request_line or "GET /generate204" in request_line:
            # Android captive portal check: redirect to main page
            response = 'HTTP/1.1 302 Found\r\nLocation: http://{}/\r\n\r\n'.format(SERVER_IP)
        elif "GET /passwd.html" in request_line:
            # Password page
            response = 'HTTP/1.0 200 OK\r\n\r\n'
            with open('passwd.html') as f:
                response += f.read()
        elif "GET /" in request_line or "GET /login.html" in request_line:
            # Main page
            response = 'HTTP/1.0 200 OK\r\n\r\n'
        ...
        try:
            ...
            if e.errno != 104:
                raise  # se non è "Connection reset", rilancia
